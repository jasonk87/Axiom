...
def parse_json_object(
    raw_text: str,
) -> tuple[dict[str, Any] | None, list[LLMValidationIssue]]:
    ...
    if not stripped:
        return None, [
            LLMValidationIssue(
                code="empty_response", message="The model response was empty."
            )
        ]
    try:
    ...
    if not isinstance(payload, dict):
        return None, [
            LLMValidationIssue(
                code="wrong_type", message="Response JSON must be an object.", path="$"
            )
        ]
    return payload, []
...
def validate_plan_payload(payload: dict[str, Any]) -> list[LLMValidationIssue]:
    ...
    if not isinstance(steps, list) or not steps:
        issues.append(
            LLMValidationIssue(
                ...
                message="The 'steps' field must be a non-empty list.",
                path="steps",
            )
        )
        return issues
    ...
    for index, step in enumerate(steps):
        ...
        if not isinstance(step, dict):
            issues.append(
                LLMValidationIssue(
                    code="wrong_type", message="Each step must be an object.", path=path
                )
            )
            continue
        ...
        if extra_keys:
            issues.append(
                LLMValidationIssue(
                    code="extra_keys",
                    message=f"Unexpected keys: {sorted(extra_keys)}.",
                    path=path,
                )
            )
        ...
        for string_key in [
            ...
            if string_key in step and not isinstance(step[string_key], str):
                issues.append(
                    LLMValidationIssue(
                        code="wrong_type",
                        message=f"Field '{string_key}' must be a string.",
        ...
        if dependencies is not None:
            if not isinstance(dependencies, list) or not all(
                ...
                issues.append(
                    LLMValidationIssue(
                        code="wrong_type",
                        message="Field 'dependencies' must be a list of strings.",
...
def validate_replan_payload(payload: dict[str, Any]) -> list[LLMValidationIssue]:
    ...
    if not isinstance(payload["is_complete"], bool):
        issues.append(
            LLMValidationIssue(
                code="wrong_type",
                message="Field 'is_complete' must be a boolean.",
                path="is_complete",
            )
        )
    ...
    if not isinstance(new_subtasks, list):
        issues.append(
            LLMValidationIssue(
                code="wrong_type",
                message="Field 'new_subtasks' must be a list.",
                path="new_subtasks",
            )
        )
        return issues
    ...
    if new_subtasks:
        for index, subtask in enumerate(new_subtasks):
            ...
            if "action" in subtask:
                if not isinstance(subtask["action"], str):
                    ...
                    issues.append(
                        LLMValidationIssue(
                            code="invalid_value",
                            message=f"Action must be one of {VALID_DECOMPOSE_ACTIONS}.",
                            path=f"{path}.action",
                        )
                    )
            ...
            for opt_key in ["target_path", "command"]:
                if (
                    opt_key in subtask
                    and subtask[opt_key] is not None
                    and not isinstance(subtask[opt_key], str)
                ):
                    issues.append(
                        LLMValidationIssue(
                            code="wrong_type",
                            message=f"Field '{opt_key}' must be a string or null.",
                            path=f"{path}.{opt_key}",
                        )
                    )
...
def validate_decompose_payload(payload: dict[str, Any]) -> list[LLMValidationIssue]:
    ...
    if not isinstance(subtasks, list) or not subtasks:
        issues.append(
            LLMValidationIssue(
                code="missing_required_field",
                message="The 'subtasks' field must be a non-empty list.",
                path="subtasks",
            )
        )
        return issues
...
def validate_review_payload(payload: dict[str, Any]) -> list[LLMValidationIssue]:
    ...
    if payload["verdict"] not in VALID_REVIEW_VERDICTS:
        issues.append(
            LLMValidationIssue(
                code="wrong_type",
                message="Field 'verdict' must be accept, revise, or caution.",
                path="verdict",
            )
        )
    ...
    if not isinstance(payload["scope_ok"], bool):
        issues.append(
            LLMValidationIssue(
                code="wrong_type",
                message="Field 'scope_ok' must be a boolean.",
                path="scope_ok",
            )
        )
    if not isinstance(payload["overbuild_detected"], bool):
        issues.append(
            LLMValidationIssue(
                code="wrong_type",
                message="Field 'overbuild_detected' must be a boolean.",
                path="overbuild_detected",
            )
        )
    if not isinstance(payload["suggested_adjustments"], list) or not all(
        isinstance(item, str) for item in payload["suggested_adjustments"]
    ):
        issues.append(
            LLMValidationIssue(
                code="wrong_type",
                message="Field 'suggested_adjustments' must be a list of strings.",
                path="suggested_adjustments",
            )
        )
    ...
    if not isinstance(findings, list):
        issues.append(
            LLMValidationIssue(
                code="wrong_type",
                message="Field 'findings' must be a list.",
                path="findings",
            )
        )
        return issues
    ...
    for index, finding in enumerate(findings):
        ...
        if "severity" in finding and finding["severity"] not in VALID_REVIEW_SEVERITIES:
            issues.append(
                LLMValidationIssue(
                    code="wrong_type",
                    message="Field 'severity' must be low, medium, or high.",
        ...
        if step_ids is not None and (
            ...
            issues.append(
                LLMValidationIssue(
                    code="wrong_type",
                    message="Field 'step_ids' must be a list of strings.",
                    path=f"{path}.step_ids",
                )
            )
    return issues
